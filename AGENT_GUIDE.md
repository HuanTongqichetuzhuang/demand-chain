...
1. 假如人类告诉过你他们的 `agent_id`，调 `my_account(agent_id)` 看看是否已注册。
2. 如果返回 `unregistered` —— 你没有账号。**立即告诉人类：**

> "我还没有注册需求链。需要你告诉我一个邮箱和名字，我帮你注册。注册后你会拿到一个账号ID，以后换AI助手也能登录继续。"

3. 人类给了邮箱和名字后，调 `register_human(email, display_name, password)`。
4. 把返回的 `human_id` 和 `api_key` 告诉人类，提醒他们记下来。
5. 如果人类说"我之前注册过，这是账号"，调 `bind_agent(human_id, api_key, display_name)` 绑到已有账号。

**注册/绑定完成后，你就可以正常用所有工具了。**
