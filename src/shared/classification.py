...
# 用于前端筛选栏和后端自动归类
CATEGORY_TREE = {
    "智能制造": ["工业机器人", "3D打印/增材制造", "数控系统", "MES/工业软件", "工业互联网", "智能仓储物流", "数字孪生"],
    "新能源": ["光伏", "风电", "氢能", "储能", "碳捕获CCUS", "核能", "生物质能", "地热能"],
    "生物医药": ["新药研发", "医疗器械", "基因编辑/治疗", "合成生物学", "生物材料", "临床试验", "体外诊断IVD", "细胞治疗"],
    "人工智能": ["大语言模型", "计算机视觉", "自然语言处理", "强化学习", "AI芯片", "具身智能", "AI安全", "多模态", "AIGC"],
    "信息技术": ["云计算", "数据安全", "物联网IoT", "区块链", "5G/6G通信", "信创", "边缘计算", "网络安全"],
    "电子科学与技术": ["微电子", "光电子", "集成电路IC", "传感器", "嵌入式系统", "MEMS", "功率半导体"],
    "材料科学": ["金属材料", "高分子材料", "复合材料", "纳米材料", "功能材料", "半导体材料", "超导材料", "智能材料"],
    "环境工程": ["水处理", "固废处理", "大气治理", "土壤修复", "环境监测", "碳中和", "噪声控制"],
    "航空航天": ["无人机", "卫星", "航天器", "航空发动机", "飞行汽车eVTOL", "空天一体化"],
    "传感器技术": ["工业传感器", "生物传感器", "环境传感器", "MEMS传感器", "光纤传感器", "图像传感器"],
    "机器人与智能系统": ["工业机器人", "服务机器人", "特种机器人", "协作机器人", "AGV/AMR", "机器视觉"],
    "农业科学": ["智慧农业", "种子育种", "植保", "农机装备", "水产养殖", "畜牧", "农业AI"],
    "化学工程": ["催化", "分离工程", "反应工程", "精细化工", "石油化工", "高分子化工"],
    "交通运输": ["新能源汽车", "轨道交通", "智能交通", "物流技术", "自动驾驶", "船舶/海运"],
    "安全科学": ["网络安全", "安全生产", "公共安全", "消防安全", "应急管理"],
    "海洋科学": ["海洋工程", "深海装备", "海洋能", "海洋生物", "海洋监测"],
    "建筑工程": ["绿色建筑", "BIM", "装配式建筑", "智能建造", "建筑材料", "结构工程"],
    "核科学": ["核能", "辐射防护", "核医学", "核探测"],
    "仪器仪表": ["分析仪器", "测试设备", "光学仪器", "精密测量", "实验室设备"],
    "生命科学": ["脑科学", "生物信息学", "合成生物学", "基因测序", "蛋白组学"],
    "量子科技": ["量子计算", "量子通信", "量子传感", "量子材料"],
    "其他": [],
}

# 所有一级分类列表（顺序与CATEGORY_TREE一致）
CATEGORY_NAMES = list(CATEGORY_TREE.keys())
import json
import logging
...
def classify_text(text: str) -> tuple[str, str]:
    ...
    if best_score == 0:
        cat_keywords = {
            "智能制造": ["制造", "加工", "装配", "生产线", "工厂", "工艺"],
            "新能源": ["能源", "发电", "电力", "可再生", "清洁能源"],
            "人工智能": ["智能", "算法", "模型", "训练", "推理", "识别"],
            "生物医药": ["药", "医", "临床", "治疗", "诊断", "病"],
            "信息技术": ["软件", "系统", "平台", "数据", "网络", "计算"],
            "环境工程": ["环境", "污染", "排放", "废水", "废气", "固废"],
            "材料科学": ["材料", "合金", "陶瓷", "纤维", "涂层"],
            "航空航天": ["航空", "航天", "飞行", "卫星", "火箭", "无人机"],
            "电子科学与技术": ["电子", "芯片", "半导体", "电路", "晶圆"],
            "传感器技术": ["传感器", "探测", "检测", "测量", "监测"],
            "机器人与智能系统": ["机器人", "自动化", "agv", "自动控制"],
            "农业科学": ["农业", "种植", "养殖", "农药", "种子", "农田"],
            "化学工程": ["化工", "化学", "催化剂", "反应器"],
            "交通运输": ["交通", "运输", "车辆", "汽车", "物流"],
            "安全科学": ["安全", "防护", "监控", "预警"],
            "建筑工程": ["建筑", "施工", "土木", "结构", "装修"],
...
CATEGORY_KEYWORDS: dict[str, dict[str, list[str]]] = {
    "智能制造": {
        "工业机器人": ["工业机器人", "机械臂", "六轴", "协作机器人", "焊接机器人", "industrial robot", "robot arm"],
        "3D打印/增材制造": ["3d打印", "增材制造", "additive manufacturing", "3d printing", "激光烧结", "光固化"],
        "数控系统": ["数控", "cnc", "加工中心", "数控系统", "cam", "数字控制"],
        "MES/工业软件": ["mes", "工业软件", "erp", "plm", "生产管理", "制造执行系统", "工业互联网平台"],
        "工业互联网": ["工业互联网", "工业物联网", "iiot", "工业4.0", "industry 4.0", "数字化工厂"],
        "智能仓储物流": ["仓储", "物流", "agv", "无人仓", "智能仓储", "分拣", "wms"],
        "数字孪生": ["数字孪生", "digital twin", "虚拟仿真", "仿真模拟", "数字映射"],
    },
    "新能源": {
        "光伏": ["光伏", "太阳能", "photovoltaic", "pv", "钙钛矿", "太阳能电池", "逆变器"],
        "风电": ["风电", "风力发电", "wind turbine", "海上风电", "offshore wind"],
        "氢能": ["氢能", "氢气", "电解水", "燃料电池", "hydrogen", "fuel cell", "制氢", "储氢"],
        "储能": ["储能", "电池", "锂离子", "钠离子", "液流", "超级电容", "battery", "energy storage"],
        "碳捕获CCUS": ["碳捕获", "ccus", "碳捕集", "碳利用", "carbon capture", "co2利用"],
        "核能": ["核能", "核电", "核反应堆", "核聚变", "nuclear", "小型堆", "smr"],
        "生物质能": ["生物质", "biomass", "沼气", "生物燃料", "生物柴油"],
        "地热能": ["地热", "geothermal", "地源热泵"],
    },
    "生物医药": {
        "新药研发": ["新药", "药物研发", "drug discovery", "创新药", "靶点", "先导化合物", "adc"],
        "医疗器械": ["医疗器械", "medical device", "影像", "介入", "支架", "手术机器人", "内窥镜"],
        "基因编辑/治疗": ["基因编辑", "crispr", "基因治疗", "基因疗法", "gene therapy", "car-t"],
        "合成生物学": ["合成生物学", "synthetic biology", "生物制造", "细胞工厂"],
        "生物材料": ["生物材料", "biomaterial", "可降解", "组织工程", "骨修复"],
        "临床试验": ["临床试验", "clinical trial", "i期", "ii期", "iii期", "fda", "nmpa"],
        "体外诊断IVD": ["诊断", "ivd", "体外诊断", "poct", "免疫诊断", "分子诊断"],
